#!/usr/bin/env python3
"""
Planet Zero YouTube Thumbnail Bot
------------------------------------
Polls the Planet Zero YouTube playlist RSS feed, detects new videos,
slugifies the title, fetches the matching image from GitHub,
and uploads it as the YouTube custom thumbnail.

Runs weekly on Wednesdays but is DISABLED between seasons.
Re-enable via the Claude scheduled tasks sidebar when Season 2 begins.
State is persisted in state/pz_last_seen.json.
"""
...
STATE_FILE = SCRIPT_DIR / "state" / "pz_last_seen.json"


def load_config():
    env_file = SCRIPT_DIR / "pz_config.env"
    if env_file.exists():
...
if missing:
    sys.exit(f"ERROR: Missing required config values: {', '.join(missing)}\n"
             f"Check {SCRIPT_DIR / 'pz_config.env'}")
...
GITHUB_THUMBNAILS_PATH = os.environ.get("GITHUB_THUMBNAILS_PATH", "planet-zero")
YOUTUBE_PLAYLIST_ID    = os.environ.get("YOUTUBE_PLAYLIST_ID", "PLd0x0jcI5YqPB_GGFup9jEe_YAwXxZhvN")
...
def main():
    log("Planet Zero Thumbnail Bot — starting")
