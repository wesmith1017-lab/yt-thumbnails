#!/usr/bin/env python3
"""
We Are Starfleet YouTube Thumbnail Bot
----------------------------------------
Polls the We Are Starfleet YouTube playlist/channel RSS feed, detects new videos,
...
State is persisted in state/was_last_seen.json.
"""
...
STATE_FILE = SCRIPT_DIR / "state" / "was_last_seen.json"


def load_config():
    env_file = SCRIPT_DIR / "was_config.env"
    if env_file.exists():
...
if missing:
    sys.exit(f"ERROR: Missing required config values: {', '.join(missing)}\n"
             f"Check {SCRIPT_DIR / 'was_config.env'}")
...
GITHUB_THUMBNAILS_PATH = os.environ.get("GITHUB_THUMBNAILS_PATH", "we-are-starfleet")
...
if not _playlist_id and not _channel_id:
    sys.exit("ERROR: Set either YOUTUBE_PLAYLIST_ID or YOUTUBE_CHANNEL_ID in was_config.env")
...
def main():
    log("We Are Starfleet Thumbnail Bot — starting")
    state = load_state()
