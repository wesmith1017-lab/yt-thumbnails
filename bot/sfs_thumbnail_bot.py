#!/usr/bin/env python3
"""
SyFy Sistas YouTube Thumbnail Bot
------------------------------------
Polls the SyFy Sistas YouTube channel RSS feed, detects new videos,
slugifies the title, fetches the matching image from GitHub,
and uploads it as the YouTube custom thumbnail.

Runs every Thursday; state is persisted in state/sfs_last_seen.json
so it skips off-weeks cleanly.
"""
...
STATE_FILE = SCRIPT_DIR / "state" / "sfs_last_seen.json"


def load_config():
    """Load sfs_config.env into environment variables."""
    env_file = SCRIPT_DIR / "sfs_config.env"
    if env_file.exists():
...
if missing:
    sys.exit(f"ERROR: Missing required config values: {', '.join(missing)}\n"
             f"Check {SCRIPT_DIR / 'sfs_config.env'}")
...
GITHUB_THUMBNAILS_PATH = os.environ.get("GITHUB_THUMBNAILS_PATH", "syfy-sistas")
YOUTUBE_PLAYLIST_ID    = os.environ.get("YOUTUBE_PLAYLIST_ID", "")

if not YOUTUBE_PLAYLIST_ID:
    sys.exit(f"ERROR: Missing required config value: YOUTUBE_PLAYLIST_ID\n"
              f"Check {SCRIPT_DIR / 'sfs_config.env'}")
