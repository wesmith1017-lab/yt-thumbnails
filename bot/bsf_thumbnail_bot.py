#!/usr/bin/env python3
"""
The BIG Sci-Fi Podcast YouTube Thumbnail Bot
----------------------------------------
Polls the The BIG Sci-Fi Podcast YouTube playlist/channel RSS feed, detects new videos,
slugifies the title, fetches the matching image from GitHub,
and uploads it as the YouTube custom thumbnail.

Episodes live in the Trek Geeks channel's BSF playlist, so this reads a playlist.
State is persisted in state/bsf_last_seen.json.
"""
...
def fetch_thumbnail(slug: str) -> tuple[bytes | None, str | None]:
    ...
    straight off disk. Otherwise it falls back to fetching over HTTPS.
    """
