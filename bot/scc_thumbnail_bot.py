#!/usr/bin/env python3
"""
Space Crime Continuum YouTube Thumbnail Bot
--------------------------------------------
Polls the SCC YouTube playlist RSS feed, detects new videos,
slugifies the title, fetches the matching image from GitHub,
and uploads it as the YouTube custom thumbnail.

Run on a schedule (daily is fine). State is persisted in
state/scc_last_seen.json so it never re-processes a video.
"""
...
STATE_FILE = SCRIPT_DIR / "state" / "scc_last_seen.json"


def load_config():
    """Load config.env into environment variables (safe — won't overwrite existing)."""
...
GITHUB_THUMBNAILS_PATH = os.environ.get("GITHUB_THUMBNAILS_PATH", "space-crime-continuum")
YOUTUBE_PLAYLIST_ID    = os.environ.get(
    "YOUTUBE_PLAYLIST_ID", "PLd0x0jcI5YqOfa5oAEvlMOvRj0cgRfjSG"
)
...
def title_to_slug(title: str) -> str:
    """
    'The Killing Game' -> 'the-killing-game'
    Lowercases, collapses runs of non-alphanumeric chars to a single hyphen.
    """
    slug = title.lower()
...
def fetch_new_videos(already_processed: list[str]) -> list[tuple[str, str]]:
    """
    Returns [(video_id, title), ...] for entries not in already_processed.
    Most-recent entry from the feed is first.
    """
...
def main():
    log("Space Crime Continuum Thumbnail Bot — starting")
    ...
    # Keep last 200 IDs to bound file size
